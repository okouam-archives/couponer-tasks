    ...
    class ShopWindow
      ...
      def find_deals
        ...
        MERCHANTS.each do |merchant|
          logger.info "Fetching datafeed for #{merchant[:name]}"
          source = open(@url % [merchant[:id]])
          gz = Zlib::GzipReader.new(source)
          result = gz.read
          #save_xml_for_tests(merchant[:filename], result)
          deal_count = Handler.new.process(result, logger, merchant[:name])
          logger.info "Finished processing deals for #{merchant[:name]}"
        end
        deal_count
      end
      ...
      def process(xml, logger, merchant_name)
        ...
        fire(:ended, deal_count, merchant_name)
      end
      ...
      def save(contents, logger, merchant_name)
        ...
        fire(:products_found, products.size, merchant_name)
        products.each do |product|
          begin
            post = Post.build(
              :post_title => "#{product['id']}: #{product.css('name').text}", 
            ...
            Category.create(post, product.css('awCat').text)
            Geography.create_from_merchant_url(post, product.css('mLink').text, merchant_name)
            post.save
            counter = counter + 1
          rescue => e
