  ...
  module Providers
    class ShopWindow
      ...
      def save(contents, merchant_name)
        ...
        fire(:deals_found, products.size, merchant_name)
        products.each do |product|
          begin
            deal_id = Couponer::Domain::Post.create(
              "#{product['id']}: #{product.css('name').text}", 
            ...
            Couponer::Domain::TermTaxonomy.assign(deal_id, {'category' => [product.css('awCat').text]})
            #TODO Implement
            #Geography.create_from_merchant_url(post, product.css('mLink').text, merchant_name)
            fire(:deal_processed)
          rescue => e
            fire(:error, product.css('name').text, product['id'], e)
          end
        end
      end
